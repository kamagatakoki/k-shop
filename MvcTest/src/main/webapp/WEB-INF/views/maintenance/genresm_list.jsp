...
<html lang="ja" dir="ltr" xmlns:th="http://www.thymeleaf.org" xmlns:layout="http://www.ultraq.net.nz/thymeleaf/layout" layout:decorator="common/layout_maintenance">
	...
	<body data-spy="scroll" data-target=".onpage-navigation" data-offset="60">
		...
		<c:import url="/WEB-INF/views/common/display_title.jsp" >
			<c:param name="title" value="小ジャンル" />
		...
		<div class="container">
			...
			<form:form modelAttribute="genreSmForm" action="list" class="form-horizontal" method="post">
				<div class="form-group">
					<div class="col-sm-4">
						<form:input path="searchGenreLgCd" class="form-control" placeholder="大ジャンルコード" />
					</div>
					<div class="col-sm-4">
						<form:input path="searchGenreMdCd" class="form-control" placeholder="中ジャンルコード" />
					</div>
					<div class="col-sm-4">
						<form:input path="searchGenreSmCd" class="form-control" placeholder="小ジャンルコード" />
					</div>
				</div>
				<div class="form-group">
					<div class="col-sm-4">
					...
					<div class="col-sm-4">
						<form:input path="searchGenreSmNm" class="form-control" placeholder="小ジャンル名称" />
					</div>
				</div>
				<div class="form-group">
					<div class="col-sm-12">
							...
							<tbody>
								...
								<c:forEach items="${items}" var="item">
									<tr>
										...
										<td><h5 class="product-title font-alt"><c:out value="${item.genreMdEntity.genreMdNm}" /></h5></td>
										<td><h5 class="product-title font-alt"><c:out value="${item.genreSmCd}" /></h5></td>
										<td><h5 class="product-title font-alt"><c:out value="${item.genreSmNm}" /></h5></td>
										<td><h5 class="product-title font-alt"><c:out value="${item.displayOrder}" /></h5></td>
										<td>
											<form:form action="/views/maintenance/genresm/list" class="form-horizontal" method="post">
												<button class="btn btn-border-d btn-circle btn-sm" type="submit" name="edit" ><i class="fa fa-cog fa-spin"></i> 編集</button>
												<input type="hidden" name="crud" value="update" />
												<input type="hidden" name="genreSmCd" value="${item.genreSmCd}" />
											</form:form>
										</td>
