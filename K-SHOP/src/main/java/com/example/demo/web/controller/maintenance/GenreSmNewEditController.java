...
import com.example.demo.service.maintenance.GenreSmService;
...
public class GenreSmNewEditController {
	@Autowired
	GenreSmService genreSmService;

	@Autowired
	GenreSmEntity genreSmEntity;
	...
	ModelAndView regsit(@Validated({ Insert.class, Default.class }) GenreSmForm genreSmForm, BindingResult bindingResult,
		...
		if (bindingResult.hasErrors()) {
			...
			modelAndView.setViewName("maintenance/genresm_newedit"); // 遷移先URLセット
		...
		BeanUtils.copyProperties(genreSmForm, genreSmEntity.getGenreLgEntity());
		BeanUtils.copyProperties(genreSmForm, genreSmEntity.getGenreMdEntity());
